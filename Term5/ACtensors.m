...
krS::usage = "\:0441\:0438\:043c\:0432\:043e\:043b \:041a\:0440\:043e\:043d\:0435\:043a\:0435\:0440\:0430";
partD::usage= "\:0447\:0430\:0441\:0442\:043d\:0430\:044f \:043f\:0440\:043e\:0438\:0437\:0432\:043e\:0434\:043d\:0430\:044f \:0434\:043b\:044f \:0442\:0435\:043d\:0437\:043e\:0440\:043d\:044b\:0445 \:043f\:043e\:043b\:0435\:0439, \:0441\:043e\:0437\:0434\:0430\:043d\:043d\:044b\:0445 \:044d\:0442\:0438\:043c \:043f\:0430\:043a\:0435\:0442\:043e\:043c";
covD::usage = "covD[T_,j_,\[CapitalGamma]] - \:043a\:043e\:0432\:0430\:0440\:0438\:0430\:043d\:0442\:043d\:043e\:0435 \:0434\:0438\:0444\:0444\:0435\:0440\:0435\:043d\:0446\:0438\:0440\:043e\:0432\:0430\:043d\:0438\:0435";
...
tensor[covD[T_,j_,\[CapitalGamma]_]]:=tensor[T] && j<=nn[basis[curBas]];
tensor[covD[T_,j_]]:=tensor[T] && j<=nn[basis[curBas]];
tensor[crS[n_Integer]]:=True;
...
type[covD[T_,j_,\[CapitalGamma]_]]/; tensor[T] && j<=nn[basis[curBas]] := type[T]+ {0,1};
type[covD[T_,j_]]/; tensor[T] && j<=nn[basis[curBas]] := type[T]+ {0,1};
type[crS[n_Integer]]:={1,1};
...
val[\[CircleTimes](T__)] := Module[{rk, pTotal, typeL, pqL, comp}, 
...
val[z[T_,j_,\[CapitalGamma]_]]:=val[\[CapitalGamma]\[CircleTimes]T];
val[pre[T_,j_,\[CapitalGamma]_,i_]]:=val[\!
\*UnderoverscriptBox[\(z[T, j, \[CapitalGamma]]\), \(2\), \(i\)]];*)
(*val[covD[T_,j_,\[CapitalGamma]_]]/;tensor[T] && j<=nn[basis[curBas]]:=val[covD[T,j]];*)
val[covD[T_,j_]]/;tensor[T] && j<=nn[basis[curBas]] :=val[partD[T,j]];
...
makeTensorField[T_, p_Integer, q_Integer] /; coor[curBas] != {} :=  
   (val[T] = Array[Underoverscript[ToString[T], StringJoin @@ ToString /@ {##} [[Range[p + 1, p + q]]], 
         StringJoin @@ ToString /@ {##} [[Range[p]]]][Sequence @@ coor[curBas]] & , ConstantArray[nn[basis[curBas]], p + q]]; 
val[partD[T,j_Integer]]  :=  Array[Underoverscript[ToString[pd[coor[curBas][[j]],T]], StringJoin @@ ToString /@ {##1}[[Range[p + 1, p + q]]], 
         StringJoin @@ ToString /@ {##1}[[Range[p]]]][Sequence @@ coor[curBas]] & , ConstantArray[nn[basis[curBas]], p + q]]; 
(*val[covD[T,j_,\[CapitalGamma]_]]/;tensor[T] && j<=nn[basis[curBas]]:= Sum[Table[Array[Underoverscript[ToString[T], StringJoin @@ ToString /@ {##1}[[Range[p + 1, p+i-1]~Join~{s}~Join~Range[p+i+1,p+q]]], 
         StringJoin @@ ToString /@ {##1}[[Range[p]]]][Sequence @@ coor[curBas]] & , ConstantArray[nn[basis[curBas]], p + q]][[1]],{i,q}],{s,p}];*)
(*val[covDDD[T,j_,\[CapitalGamma]_]]/;tensor[T] && j<=nn[basis[curBas]]:= Sum[Sum[Array[Underoverscript[ToString[\[CapitalGamma]],StringJoin @@ ToString /@ {j}~Join~{##}[[{p}]],ToString[s]]
		Underoverscript[ToString[T], StringJoin @@ ToString /@ {##2}[[Range[p + 1, p+i-1]]]~Join~{s}~Join~{##1}[[Range[p+i+1,p+q]]], 
         StringJoin @@ ToString /@ {##1}[[Range[p]]]][Sequence @@ coor[curBas]] & , ConstantArray[nn[basis[curBas]], p + q]],{s,nn[basis[curBas]]}],{i,q}];
val[covDD[T,j_,\[CapitalGamma]_]]/;tensor[T] && j<=nn[basis[curBas]]:=-Sum[Sum[Array[Underoverscript[ToString[\[CapitalGamma]],ToString[j]<>ToString[s],StringJoin @@ ToString /@ {##}[[Range[1]]]]Underoverscript[ToString[T], StringJoin @@ ToString /@ {##1}[[Range[p + 1, p + q]]], 
         StringJoin @@ ToString /@ {##}[[Range[i-1]]]~Join~{s}~Join~{##1}[[Range[i+1,p]]]][Sequence @@ coor[curBas]] & , ConstantArray[nn[basis[curBas]], p + q]],
{i,p}],{s,nn[basis[curBas]]}];*)
val[covD[T,j_,\[CapitalGamma]_]]/;tensor[T] && j<=nn[basis[curBas]]:=Array[Underoverscript[ToString[pd[coor[curBas][[j]],T]], StringJoin @@ ToString /@ {##1}[[Range[p + 1, p + q]]], 
         StringJoin @@ ToString /@ {##1}[[Range[p]]]][Sequence @@ coor[curBas]] & , ConstantArray[nn[basis[curBas]], p + q]]+
Sum[Sum[Array[Underoverscript[ToString[\[CapitalGamma]],StringJoin @@ ToString /@ {j}~Join~{##}[[{1}]],ToString[s]]Underoverscript[ToString[T], StringJoin @@ ToString /@ {##2}[[Range[p + 1, p+i-1]]]~Join~{s}~Join~{##1}[[Range[p+i+1,p+q]]], 
         StringJoin @@ ToString /@ {##1}[[Range[p]]]][Sequence @@ coor[curBas]] & , ConstantArray[nn[basis[curBas]], p + q]],{s,nn[basis[curBas]]}],{i,q}]-
Sum[Sum[Array[Underoverscript[ToString[\[CapitalGamma]],ToString[j]<>ToString[s],StringJoin @@ ToString /@ {##}[[{1}]]]Underoverscript[ToString[T], StringJoin @@ ToString /@ {##1}[[Range[p + 1, p + q]]], 
         StringJoin @@ ToString /@ {##}[[Range[i-1]]]~Join~{s}~Join~{##1}[[Range[i+1,p]]]][Sequence @@ coor[curBas]] & , ConstantArray[nn[basis[curBas]], p + q]],
{i,p}],{s,nn[basis[curBas]]}];
...
Unprotect[allRules, allDefRules]; 
